var TWmap = {
  container : d3.select(document.getElementById("map")),
  map : d3.select(document.getElementById("map")),
  map_container : d3.select(document.getElementById("map_container")),
  idx : {
    convert : {}
  },
  height : 600,
  color : d3.scale.linear().domain([0,0.5,1]).range(["#0000ff","#ffffff","#00ff00"]),
  global_data : {},
  get : function(s){
    if(s[0]=="#")
      return document.getElementById(s.substr(1));
  },
  projection : d3.geo.mercator().scale(50000).translate([-16750, 3350]),
  centered : null,
...
(function(){
  TWmap.width = (function(s){
    return parseInt(s.substr(0,s.length-2));
  })(TWmap.map_container.style("width"));
